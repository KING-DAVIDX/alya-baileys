...
const makeGroupsSocket = (config) => {
    ...
    return {
        ...
        groupParticipantsUpdate: async (jid, participants, action) => {
            // Ensure sessions exist for all participants
            await assertSessions(
                participants.map(p => (0, WABinary_1.jidNormalizedUser)(p)), 
                false
            );
...
const extractGroupMetadata = (result) => {
    ...
    const metadata = {
        ...
        participants: (0, WABinary_1.getBinaryNodeChildren)(group, 'participant').map(({ attrs }) => {
            const isLid = attrs.jid && attrs.jid.includes(':lid');
            return {
                id: isLid ? attrs.jid : (attrs.phone_number ?? attrs.jid),
                admin: (attrs.type || null),
                lid: attrs.lid || attrs.jid || undefined
            };
        }),
